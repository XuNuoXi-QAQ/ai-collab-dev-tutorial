...
function detectLanguage() {
  const language = import_obsidian.moment.locale();
...
var Metadata = class {
   ...
   */
  async getAttachFileName(setting, dateFormat, originalName, adapter) {
...
function isOriginalNameVariable(setting, ext) {
  const { extSetting } = getExtensionOverrideSetting(ext, setting);
  if (extSetting !== void 0 && extSetting.attachFormat.trim() === SETTINGS_VARIABLES_ORIGINALNAME || setting.attachFormat.trim() === SETTINGS_VARIABLES_ORIGINALNAME) {
    return true;
  }
  return false;
}
...
var ArrangeHandler = class {
  ...
  async rearrangeAttachment(type, file, oldPath) {
    ...
    for (const obNote of Object.keys(attachments)) {
      ...
      if (!(innerFile instanceof import_obsidian9.TFile) || isAttachment(this.app, this.settings, innerFile)) {
  ...
  async getAttachmentsInVaultByLinks(settings, type, file, oldPath) {
    ...
    if (type == 1 /* LINKS */) {
      ...
      if (file2) {
        if (file2.parent && isExcluded(file2.parent.path, this.settings) || isAttachment(this.app, this.settings, file2)) {
          allFiles = [];
        } else {
          ...
          for (const k of Object.keys(frontmatter)) {
            if (typeof frontmatter[k] === "string") {
              ...
              if (formatMatch && formatMatch[1]) {
                ...
                if (file2 && isAttachment(this.app, settings, file2.path)) {
  ...
  onOpen() {
    ...
    new import_obsidian10.Setting(contentEl).addButton((btn) => {
      ...
      (btn) => btn.setButtonText(t("confirm.continue")).setWarning().onClick(() => {
        new ArrangeHandler(this.plugin.settings, this.plugin.app, this.plugin).rearrangeAttachment(1 /* LINKS */).then(() => new import_obsidian10.Notice(t("notifications.arrangeCompleted"))).catch((err) => {
          var _a;
