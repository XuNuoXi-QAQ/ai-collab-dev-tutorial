...
开发：蚕子 QQ：312815311 更新时间：2021-12-30
***************************************************************************** */
...
var MyPlugin = /** @class */ (function (_super) {
        ...
        function() {
            ...
            function(_a) {
		        ...
		        _this.addCommand({
		            id: 'change-name-path',
		            name: '重命名并去除路径',
		            callback: function() { _this.重命名并去除路径(); }
		        });
                _this.addCommand({
		            id: 'change-name',
		            name: '重命名不去除路径',
		            callback: function() { _this.重命名不去除路径(); }
		        });
                _this.addCommand({
		            id: 'fan-jian',
		            name: '繁体转简',
    ...
    MyPlugin.prototype.重命名并去除路径 = function() {
        ...
        var regline = line0.match(/(?<=\[\[).*?(?=\]\])/g);
        console.log("regline", regline);
        if (regline == null) {
            console.log("not have reg");
            return false
        }
        var line1 = line0
        for (var i=0; i<regline.length; i++)
        {
        	//console.log("regline", regline[i]);
        	var list = regline[i].split("/");
        	//console.log(list);
        	var tmp = list[list.length - 1].split("|")[0];
        	//console.log(tmp.split("|")[0]);
        	var tmp1 = tmp.concat("\|" ,tmp.toString());
        	//console.log("tmp1", tmp1);
        	line1 = line1.replace(regline[i], tmp1);
        	//console.log("line1", line1);
        	//console.log(tmp1);
        	//console.log(regline[i].split("/"));
        }
        //console.log(line1);
		//console.log(line0.match(/(?<=\[\[).*?(?=\]\])/g));
		//console.log("aaa");
		doc.replaceRange(line1, {line:cursorline,ch:0},{line:cursorline,ch:line0.length});
    };

    MyPlugin.prototype.重命名不去除路径 = function() {
		var mdView = this.app.workspace.activeLeaf.view;
        if (mdView.sourceMode == undefined)
            return false;
        var cmEditor = mdView.sourceMode.cmEditor;
        ...
        for (var i=0; i<regline.length; i++)
        {
        	...
        	var list = regline[i].split("/");
        	//console.log(list);
        	var name = list[list.length - 1].split("|")[0];
        	//console.log(tmp.split("|")[0]);
        	var tmp1 = regline[i].split("|")[0].concat("\|" ,name.toString());
        	//console.log("tmp1", tmp1);
        	line1 = line1.replace(regline[i], tmp1);
        	//console.log("line1", line1);
        	//console.log(tmp1);
...
var SettingsTab = /** @class */ (function (_super) {
    ...
    SettingsTab.prototype.display = function () {
        ...
        containerEl.empty();
        containerEl.createEl('h2', { text: '增强编辑 0.3.3' });
        ...
        donateText.appendChild(document.createElement('br'));
        donateText.appendText('转换待办状态　　转换选文行首的待办状态，顺序为 -[ x-!?><+] 效果');
        donateText.appendChild(document.createElement('br'));
        donateText.appendChild(document.createElement('br'));
        donateText.appendText('转换文字颜色　　Ctrl+Shift+ 1-7　　将选文转为或去除 赤橙黄绿青蓝紫 颜色');
        donateText.appendChild(document.createElement('br'));
        donateText.appendText('转换文字颜色　　Ctrl+Alt+ 1-7　　将选文背景转为或去除 赤橙黄绿青蓝紫 颜色');
        donateText.appendChild(document.createElement('br'));
        donateText.appendChild(document.createElement('br'));
